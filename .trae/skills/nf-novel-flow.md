...
|------|------|------|--------|------|
| topic | string | 是 | - | 创作主题 |
| market-research | boolean | 否 | false | 是否进行市场调研 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| concept | string | 是 | - | 选定概念 |
| depth | string | 否 | basic | 规划深度 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| outline | string | 是 | - | 大纲文件 |
| chapters | string | 否 | - | 章节范围 |
...
```
/nf-novel-flow merge --volumes <volume-files> --output <output-path>
```
...
### 模式二：并行分卷创作模式 (Parallel Volume Mode)
...
| **Split**          | **流程协调**       | **nf-process-coordinator**     | **nf-volume-manager**                                                          |
| **Parallel Build** | **内容生产部(xN)** | **nf-content-production-dept** | **nf-parallel-executor, nf-content-generator**                                 |
| **Merge**          | **流程协调**       | **nf-process-coordinator**     | **nf-novel-merger**                                                            |
