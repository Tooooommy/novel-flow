...
| volumes  | number | auto   | 分卷数量（自动计算）   |
| chapters | number | auto   | 每卷章节数（自动计算） |

**自动计算规则**：

| 场景                 | 计算逻辑                                                     |
| -------------------- | ------------------------------------------------------------ |
| 只给 target          | volumes = target/100万（最小1），chapters = 100（每卷100章） |
| 只给 target+volumes  | chapters = target/volumes/4000（每章4000字）                 |
| 只给 target+chapters | volumes = target/chapters/4000                               |
| 全给                 | 使用指定值                                                   |
